  ...
  setup(props) {
    ...
    const drawMap = () => {    
      ...
      if (props.pngArray != undefined) {
        (
          ...
          map.addSource(item.visualId, {
            type: "image",
            url: `${process.env.VUE_APP_BACK_ADDRESS}visual/getPngResource/${item.visualId}`,
      ...
      if (movePngArray.value.length > 0) {
        map.addSource(movePngArray.value[0].visualId, {
          type: "image",
          url: `${process.env.VUE_APP_BACK_ADDRESS}visual/getPngResource/${movePngArray.value[0].visualId}`,
    ...
    const changeRaster = (handle: string) => {
      ...
      map.addSource(movePngArray.value[centerIndex.value].visualId, {
        type: "image",
        url: `${process.env.VUE_APP_BACK_ADDRESS}visual/getPngResource/${
