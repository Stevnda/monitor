        ...
        if (
          ...
          map.addSource(param.id, {
            ...
            tiles: [window.location.origin + 
              `${process.env.VUE_APP_BACK_ADDRESS}visual/getVectorTiles/${param.visualId}/{x}/{y}/{z}`,
            ],
          });
          map.addLayer({
            id: param.id,
          ...
          map.addSource(param.id, {
            type: "raster",
            tiles: [window.location.origin + 
              `${process.env.VUE_APP_BACK_ADDRESS}visual/getRaster/${param.visualId}/{x}/{y}/{z}`,
            ],
          });
          map.addLayer({
            id: param.id,
          ...
          if (coordinates != null && (coordinates as any).code === 0) {
            map.addSource(param.id, {
              type: "image",
              url: window.location.origin + `${process.env.VUE_APP_BACK_ADDRESS}visual/getPngResource/${param.visualId}`,
